...
function ShopPage() {
  ...
  return (
    <main>
      <PageIntro
        number="02"
        eyebrow="The ORA edit"
        title={
          <>
            Objects for
            <br />
            <em>all your selves.</em>
          </>
        }
        description="A signature, a small rebellion, a story at your wrist. Explore the collection here. Find current prices, availability and checkout at ORA."
        image="/media/bangles.jpg"
        caption="Collected over time. Worn together."
      />
      <div className="editorial-container lookbook-body">
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {collectionItems.map((c) => (
            ...
            >
              ...
              <div className="p-5">
                <p className="font-sans text-[0.65rem] uppercase tracking-[0.16em] text-metal">
                  {c.tag}
                </p>
        ...
        <section className="mt-20">
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {filtered.slice(0, limit).map((p) => (
              ...
              >
                ...
                <div className="p-4">
                  <p className="font-sans text-[0.6rem] uppercase tracking-[0.16em] text-metal">
                    {p.category}
                  </p>
                  <h3 className="mt-1 font-serif text-lg leading-snug">{p.name}</h3>
                  <p className="mt-2 text-xs text-muted">View on ORA ↗</p>
                </div>
              </a>
            ))}
          </div>
          {filtered.length === 0 && (
            <p className="py-10 text-muted" role="status">
              No pieces match this search. Try a different word or category.
            </p>
          )}
          {filtered.length > limit && (
            <button className="editorial-button mt-8" onClick={() => setLimit(limit + 24)}>
              Show more pieces
            </button>
          )}
        </section>
